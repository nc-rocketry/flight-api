...
public class SearchController extends TemplateController {
    ...
    @GetMapping(value = "/diameter/{diameter}", produces = "text/html")
    public String byDiameter(@PathVariable float diameter, Model model) {
        init();

        List<MotorDTO> motors = motorCache.getDiameter(diameter).getMotors().stream()
                .map(motor -> new MotorDTO(motor))
                .collect(Collectors.toList());

        set("motors", motors);
        set("selected_diameter", diameter);
